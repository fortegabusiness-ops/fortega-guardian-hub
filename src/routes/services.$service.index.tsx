...
import { areasForService } from "@/lib/seo/service-areas";
import {
...
function ServiceDetailPage() {
  ...
  const industries = service.industriesServed
    ...
    .filter(Boolean);
  const cityPages = areasForService(service.slug);
        ...
        <section className="border-t border-border">
          <div className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-20">
            ...
            <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
              Local pages covering licensing, conditions and coverage in each market.
            </p>
            <div className="mt-6 flex flex-wrap gap-2">
              {cityPages.map((a) => (
                <Link
                  ...
                  className="rounded-full border border-border bg-surface/40 px-4 py-1.5 text-sm text-foreground hover:border-brand-glow/60"
                >
                  {a.city.name}
                </Link>
              ))}
              <Link
                to="/locations"
                className="rounded-full border border-border bg-surface/40 px-4 py-1.5 text-sm font-medium text-brand-glow hover:border-brand-glow/60"
              >
                All locations
              </Link>
            </div>
